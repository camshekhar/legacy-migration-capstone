...
| Variable | Purpose |
|---|---|
| `LLM_API_KEY` | Google AI Studio (Gemini) API key used by all LangChain agents |
| `LLM_MODEL` | Claude model name |
| `SOURCE_DB_URL` | SQLAlchemy URL for the legacy MySQL database |
| `SNOWFLAKE_*` | Target warehouse connection details |
| `LANGFUSE_*` | Observability project keys |
| `CONFIDENCE_THRESHOLD` | Mappings below this require human review (default 0.80) |
| `TABLE_FILTER` | `all` or a comma-separated subset of source tables |

## 5. How to Run the Migration Pipeline
...
## 6. How to Rollback a Failed Migration
...
## 7. How to Inspect AI Decisions (Audit Log + LangFuse)
